...
def remove_kth_last_node(head: ListNode, k: int) -> ListNode:
    ...
    return dummy.next
Complexity Analysis
Time complexity: The time complexity of remove_kth_last_node is 
O
(
n
)
O(n). This is because the algorithm first traverses at most 
n
n nodes of the linked list, and then two pointers traverse the linked list at most once each.
