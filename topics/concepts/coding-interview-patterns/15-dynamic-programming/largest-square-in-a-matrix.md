...
def largest_square_in_a_matrix(matrix: List[List[int]]) -> int:
    ...
    return max_len ** 2
Complexity Analysis
Time complexity: The time complexity of largest_square_in_a_matrix is 
O
(
m
⋅
...
O(m⋅n) because each cell of the DP table is populated once.
...
O(m). Below is the optimized code:
